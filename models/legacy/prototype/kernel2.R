...
#---------------------------
cat("Preparing data...\n")
tr2 <- as.data.table(tr_te[tri, ])
te2 <- as.data.table(tr_te[-tri, ])

y_tr <- y[tri]
y_te <- y[-tri]

train_processed <- tr2[1:240000]
val_processed <- tr2[240001:nrow(tr2)]
test_processed <- te2

train_processed[, TARGET := y_tr[1:240000]]
val_processed[, TARGET := y_tr[240001:nrow(tr2)]]

tr2[, TARGET := y_tr]

fwrite(train_processed, "data/val_train.csv", row.names = F)
fwrite(val_processed, "data/val_test.csv", row.names = F)
fwrite(tr2, "data/full_train.csv", row.names = F)
fwrite(test_processed, "data/full_test.csv", row.names = F)
